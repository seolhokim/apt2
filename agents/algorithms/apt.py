...
class APTAgent:
    def __init__(self,
        ...
        self.projection_opt = torch.optim.Adam(self.projection.parameters(), lr=lr)
    ...
    def train(self, training=True):
        ...
        self.projection.train(training)
    ...
    def compute_reward(self, next_obs):
        ...
        representation = self.critic.get_representation(next_obs)
        reward = self.pbe(representation).reshape(-1,1)
        return reward
    
    def update_representation(self, obs, step):
        metrics = dict()
    ...
    def update(self, replay_iter, step):
        ...
        obs_, action, extr_reward, discount, next_obs_, done = utils.to_torch(
            batch, self.device)
        if self.reward_free : 
            metrics.update(self.update_representation(obs_, step))
            
            intr_reward = self.compute_reward(next_obs_).detach()
            reward = intr_reward
        else :
            reward = extr_reward
        if self.use_tb or self.use_wandb:
